...
export async function GET(request: Request) {
  try {
    ...
    // Consultar las abejas asociadas al userId en la colección `BEES`
    const beesRef = collection(db, 'BEES');
    const beesQuery = query(beesRef, where('userId', '==', userId));
    const beesSnapshot = await getDocs(beesQuery);

    if (beesSnapshot.empty) {
      return NextResponse.json({ error: 'No bees found for the user' }, { status: 404 });
    }

    // Construir la estructura de respuesta con las abejas, partes y slots
    const beePromises = beesSnapshot.docs.map(async (beeDoc, index) => {
      ...
      const partsPromises = partIds.map(async (partId: string) => {
        ...
        const partData = partDoc.exists() ? partDoc.data() : null;
    ...
    if (bees.length === 0) {
      return NextResponse.json({ error: 'No eligible bees found' }, { status: 404 });
    }

    // Devolver la estructura completa con las abejas, partes y slots
    return NextResponse.json({ bees }, { status: 200 });
  } catch (error) {
