...
export async function createBeeParts(userId: string, hash: string) {
  try {
    // Verificar que los datos sean válidos
    if (!userId || !hash) {
      throw new Error('userId and hash are required');
    }
    ...
    const majorityType = uniqueTypes === parts.length ? 'All' : Object.keys(typeCount).reduce((a, b) => typeCount[a] > typeCount[b] ? a : b);
